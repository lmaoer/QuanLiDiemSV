...
<body>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Loại điểm', 'Phần trăm'],
                ['A: >= 8,5đ', <%=thongke.findAByMon("NN1")%>],
                ['B: >= 7đ', <%=thongke.findBByMon("NN1")%>],
                ['C: >= 5.5đ', <%=thongke.findCByMon("NN1")%>],
                ['D: >= 4đ', <%=thongke.findDByMon("NN1")%>],
                ['F: < 4đ', <%=thongke.findFByMon("NN1")%>]
            ]);
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Loại điểm', 'Phần trăm'],
                ['A: >= 8,5đ', <%=thongke.findAByMon("NN2")%>],
                ['B: >= 7đ', <%=thongke.findBByMon("NN2")%>],
                ['C: >= 5.5đ', <%=thongke.findCByMon("NN2")%>],
                ['D: >= 4đ', <%=thongke.findDByMon("NN2")%>],
                ['F: < 4đ', <%=thongke.findFByMon("NN2")%>]
            ]);
