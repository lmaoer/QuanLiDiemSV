...
<body>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Loại điểm', 'Phần trăm'],
                ['A: >= 8,5đ', <%=thongke.findAByMon("KT1")%>],
                ['B: >= 7đ', <%=thongke.findBByMon("KT1")%>],
                ['C: >= 5.5đ', <%=thongke.findCByMon("KT1")%>],
                ['D: >= 4đ', <%=thongke.findDByMon("KT1")%>],
                ['F: < 4đ', <%=thongke.findFByMon("KT1")%>]
            ]);
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Loại điểm', 'Phần trăm'],
                ['A: >= 8,5đ', <%=thongke.findAByMon("KT2")%>],
                ['B: >= 7đ', <%=thongke.findBByMon("KT2")%>],
                ['C: >= 5.5đ', <%=thongke.findCByMon("KT2")%>],
                ['D: >= 4đ', <%=thongke.findDByMon("KT2")%>],
                ['F: < 4đ', <%=thongke.findFByMon("KT2")%>]
            ]);
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Loại điểm', 'Phần trăm'],
                ['A: >= 8,5đ', <%=thongke.findAByMon("KT3")%>],
                ['B: >= 7đ', <%=thongke.findBByMon("KT3")%>],
                ['C: >= 5.5đ', <%=thongke.findCByMon("KT3")%>],
                ['D: >= 4đ', <%=thongke.findDByMon("KT3")%>],
                ['F: < 4đ', <%=thongke.findFByMon("KT3")%>]
            ]);

            var options = {
                title: "NGUYÊN LÝ THỐNG KÊ",
                pieHole: 0.4,
            };
